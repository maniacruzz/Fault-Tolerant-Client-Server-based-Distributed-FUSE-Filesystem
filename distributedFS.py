...
class Memory(LoggingMixIn, Operations):
    ...
    def traverse(self, path, tdata = False, handle="open", update={}, bid=0):
        ...
        if(not with_server):
            ...
            if(with_server and handle=="close"):# and tdata == False):
                if(ddbug): print("server traverse in close for meta")
    ...
    def create(self, path, mode):
        ...
        p['files'][tar] = dict(st_mode=(S_IFREG | mode), st_nlink=1,
                     ...
                     st_atime=time(), start_d=random.randrange(0,self.num_D))
        self.fd += 1
    ...
    print("-------after push---------pn",pn)
    print("-------after pop---------po",po)
    ...
    print("----------------dn1",dn1)
        ...
        self.traverseparent( old, False, "close",po)
        self.traverseparent( new, False, "close",pn)
        self.traverseparent( old, True, "close",do)
        self.traverseparent( new, True, "close",dn)'''
