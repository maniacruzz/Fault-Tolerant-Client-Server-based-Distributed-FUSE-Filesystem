...
class SimpleHT:
    ...
    def traverse(self, path, tdata = False, handle="open", update={}, bid=0, tred = False):
        ...
        if(ddbug):print("traverse started")
  ...
  # Retrieve something from the HT
    def get(self, key):
        ...
        if(cmd['function']=="traverseparent"):# and cmd['handle']=="open"):
          ...
          rv=self.traverse(cmd['path'], cmd['tdata'], cmd['handle'], cmd['update'], cmd['bid'],cmd['tred'])
  ...
  # #Insert something into the HT
    def put(self, key, value):
        ...
        if(cmd["function"]=="traverseparent"):
            ...
            if(ddbug):print("server traverse with cmd", cmd)
            self.traverse(cmd['path'], cmd['tdata'], cmd['handle'], cmd['update'], cmd['bid'],cmd['tred'])
        return True
  ...
  # Load contents from a file
    ...
    def corrupt(self, path, bid=0, tred=False, cid=0, cchar='*'):
        ...
        try:
            d = self.traverse(path,True,"open", {},bid, tred)
            print("fetched before corruption  "+d[0]+" with checksum :"+str(d[1]))
            if cid<len(d[0])-1:
		if d[0][cid]==cchar:
			cchar=chr(ord(cchar)+1)
                d[0]=d[0][:cid]+cchar+d[0][cid+1:]
            print("updating after corruption  "+d[0]+" with checksum :"+str(d[1]))
            self.traverse(path,True,"close", d,bid, tred)
            
            d = self.traverse(path,True,"open", {}, bid, tred)
            print("re fetching corrupted block "+d[0]+" with checksum :"+str(d[1]))
        except IndexError:
